...
import {
  ...
  MapPin, Wifi, ChevronDown, ChevronRight, Zap, 
...
const CLASSIFICATION_CONFIG = {
  likely_independent: {
    ...
    bg: "bg-[hsl(var(--success-muted,var(--secondary)))] border-[hsl(var(--success))]",
  },
  likely_chain: {
    label: "Likely Chain",
    icon: Building2,
  ...
  unknown: {
    ...
    bg: "bg-[hsl(var(--warning-muted,var(--secondary)))] border-[hsl(var(--warning))]",
  },
};
...
export function HotelIntelligenceDrawer({ hotel, nearbyHotels, cityCenter, onClose, onDealCreated }: Props) {
  ...
  const handleCreateDeal = async () => {
    ...
    try {
      ...
      toast({ title: "✅ Deal created!", description: `${dealName} added to pipeline. Opening feasibility…` });
      onDealCreated(data);
    } catch (err: any) {
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-[499] bg-black/30 lg:hidden"
        onClick={onClose}
      />
      {/* Drawer — full-screen on mobile, 420px panel on desktop */}
      <div className="fixed inset-x-0 bottom-0 top-12 z-[500] flex flex-col bg-background border-t border-border shadow-2xl overflow-hidden lg:inset-x-auto lg:right-0 lg:top-0 lg:w-[420px] lg:border-l lg:border-t-0">
        {/* Header */}
        <div className="flex items-start justify-between p-4 border-b border-border shrink-0">
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-sm font-semibold leading-tight">{hotel.name}</h2>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              {hotel.stars !== undefined && (
                <div className="flex items-center gap-1.5">
                  <Star className="h-3.5 w-3.5 text-[hsl(var(--aurora-gold))] shrink-0" />
                  <span className="text-xs">{hotel.stars} stars</span>
                </div>
              )}
              {hotel.rooms !== undefined && (
                <div className="flex items-center gap-1.5">
                  <BedDouble className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                  <span className="text-xs">{hotel.rooms} rooms</span>
                </div>
              )}
              {hotel.hasWebsite && (
          ...
          <div className="p-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-muted/50 rounded-lg p-3 text-center">
                ...
                <div className="text-[10px] text-muted-foreground">Competitors &lt;3km</div>
              </div>
              {distToCity !== null && (
                <div className="bg-muted/50 rounded-lg p-3 text-center">
                  <div className="text-lg font-bold">{distToCity.toFixed(1)} km</div>
                  <div className="text-[10px] text-muted-foreground">From city center</div>
                </div>
              )}
            </div>
          ...
          <div className="p-4 space-y-3">
            ...
            {knownOwnerTags.length > 0 ? (
              <div className="space-y-1">
                {hotel.tags.operator && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-xs">{hotel.tags.operator}</span>
                ...
                {hotel.tags.brand && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-xs">{hotel.tags.brand}</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground bg-secondary border border-border rounded-lg p-2.5">
              ...
              <div className="mt-2 max-h-36 overflow-y-auto space-y-0.5">
                {Object.entries(hotel.tags).map(([k, v]) => (
                  <div key={k} className="flex gap-2 text-xs py-0.5">
                    <span className="text-muted-foreground font-mono w-28 shrink-0 truncate">{k}</span>
        ...
        <div className="p-4 border-t border-border bg-background shrink-0">
          ...
          <p className="text-[10px] text-muted-foreground text-center mt-1.5">
            Creates deal + opens Feasibility tab automatically
          </p>
        </div>
      </div>
    </>
